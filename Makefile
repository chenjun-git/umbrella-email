...
CURRENT_GIT_REPO := umbrella-email
...
build: folder_dep
	$(BUILDENVVAR) go build -o $(GOBIN)/umbrella-email -ldflags "-X main.BuildTime=`date '+%Y-%m-%d_%I:%M:%S%p'` -X main.BuildGitHash=`git rev-parse HEAD` -X main.BuildGitTag=`git describe --tags`" $(CURRENT_GIT_GROUP)/$(CURRENT_GIT_REPO)
...
test: folder_dep
	go test $(CURRENT_GIT_GROUP)/$(CURRENT_GIT_REPO)/directmail/smtp
	go test $(CURRENT_GIT_GROUP)/$(CURRENT_GIT_REPO)/directmail
